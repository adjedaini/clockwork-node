        ...
        <div className="flex items-center gap-3 mb-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-slate-900 border border-slate-700 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
            />
          </div>
          <button
            onClick={onRefresh}
          ...
          {methods.map((method) => (
            <button
              ...
              className={`px-3 py-1 rounded-lg text-xs font-medium transition-colors ${
                methodFilter === method ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'
              }`}
            >
              {method}
            </button>
